...
| **Historické audity a další poznámky** | ostatní soubory v `docs/` |
...
- **Health:** `GET http://localhost:8080/actuator/health` a `GET /health`
...
```bash
cd backend-java && ./gradlew test        # JUnit 5
...
2. **Vlastní build** — skripty v `scripts/` a `tools/` (indexace a evaluace); postup a struktura
   indexu jsou v [`docs/FTS_AND_SIDECAR.md`](docs/FTS_AND_SIDECAR.md) a
   `docs/search_v2_fts_index_audit.md`.
...
├─ docs/                    # mapy + technické audity (viz tabulka nahoře)
...
záměrně jen částečné — konkrétní stav a poznámky jsou v `docs/` a v historických auditech.
